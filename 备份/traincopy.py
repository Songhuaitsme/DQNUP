...
def train():
    ...
    for time_step in range(config.MAX_STEPS):
        ...
        # 策略A: 简单随机 (0 到 MAX 个任务)
        # if random.random() < config.TASK_GENERATION_PROB:
        #     current_batch_size = random.randint(1, config.MAX_TASKS_PER_STEP)
        # 策略B: 泊松分布 (更符合真实流量，有时0个，有时多个)
        current_batch_size = np.random.poisson(config.TASK_ARRIVAL_RATE)

        # 限制一下最大值，防止显存爆炸或死循环 (可选)
        ...
        for _ in range(current_batch_size):
            ...
            # 获取当前状态 (注意：同一个 time_step 内，前一个任务分配后，状态会立即改变，供下一个任务决策)
            ...
            # 训练 (每次决策后都训练一次，或者你也可以移到 batch 循环外面去每步只训练一次)
            loss = agent.replay()
            ...
            if info['status'] == 'Success':
                ...
                active_tasks.append({
                    'end_time': time_step + new_task['duration'],  # 结束时间基于当前 time_step
                    'node_id': action_node,
                    'cpu': new_task['cpu'],
                    'bw': new_task['bw'],
                    'path': info['path']
